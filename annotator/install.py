...
from annotator.usefull_function import command_required_option_from_option, multiprocessing_download, get_install_mode
...
def create_bash_completion():
    ...
    if major < "4" or (major == "4" and minor < "4"):
        ...
        if not Path(f"{ANNOTATOR_PATH}/annotator-complete.sh").exists():
            build_completion = f"_ANNOTATOR_COMPLETE=bash_source annotator > {ANNOTATOR_PATH}/annotator-complete.sh"
            os.system(build_completion)
        with open(bashrc_file, "r") as bash_file_read:
            if not [True for line in bash_file_read if "ANNOTATOR" in line]:
                with open(bashrc_file, "a") as bash_file_open:
                    append_bashrc = f"\n#Add autocompletion for ANNOTATOR\n. {ANNOTATOR_PATH}/annotator-complete.sh"
                    bash_file_open.write(append_bashrc)
                    click.secho(f"INSTALL autocompletion for ANNOTATOR on {bashrc_file} with command {append_bashrc}\nUpdate with commande:\nsource ~/.bashrc",
                                fg="yellow")
            else:
                click.secho(f"warning autocompletion for ANNOTATOR  already found on {bashrc_file} please check path",
                            fg="yellow")


def create_envmodules(modules_dir):
    from annotator import MODULE_FILE
    modules_dir = Path(modules_dir)
    modules_dir.mkdir(parents=True, exist_ok=True)
    modules_dir.joinpath(f"{annotator.__version__}").open("w").write(MODULE_FILE)
    click.edit(require_save=False, extension='', filename=modules_dir.joinpath(f"{annotator.__version__}").as_posix())
    click.secho(f"\n    Success install module file for version {annotator.__version__} on path '{modules_dir}'", fg="yellow")




@click.command("install_cluster", short_help='Install annotator on HPC cluster', context_settings=dict(max_content_width=800),
...
@click.option('--bash_completion/--no-bash_completion', is_flag=True, required=False, default=True, show_default=True,
              help='Allow bash completion of annotator commands on the bashrc file')
@click.option('--create_envmodule/--no-create_enmodules', is_flag=True, required=False, default=False, show_default=True,
              help='Create a env module file allowing load annotator in a cluster')
...
def install_cluster(scheduler, env, bash_completion, create_envmodule, modules_dir):
    ...
    mode_file = ANNOTATOR_MODE
    # build default profile path
    default_profile = ANNOTATOR_PROFILE
    def fail():
    ...
    try:
        ...
        default_cluster = ANNOTATOR_PATH.joinpath("install_files",f"cluster_config_{scheduler.upper()}.yaml")
        ...
        try:
            ...
            if click.confirm(click.style(f'Now Edit file tools_path.yaml according to your HPC:\nMore info on {DOCS}\n(enter to continue)', fg="blue"), default=True, abort=True, show_default=True):
                click.edit(require_save=False, extension='.yaml', filename=f"{ANNOTATOR_TOOLS_PATH}")
    ...
    try:
        ...
        click.secho(f"\n    Congratulations, you have successfully installed annotator !!!\n\n", fg="green", bold=True)
...
@click.command("install_local", short_help='Install annotator on local computer', context_settings=dict(max_content_width=800), no_args_is_help=False)
@click.option('--bash_completion/--no-bash_completion', is_flag=True, required=False, default=True, show_default=True,
              help='Allow bash completion of annotator commands on the bashrc file')
...
@click.command("test_install", short_help='test annotator with data_test after install (local or cluster)', context_settings=dict(max_content_width=800), no_args_is_help=False)
...
def test_install(data_dir):
    ...
    txt =  ANNOTATOR_CONFIG_PATH.open("r").read().replace("./Data-Xoo-sub",f"{data_dir}/Data-Xoo-sub").replace("./annotator_OUTPUT/",f"{data_dir}/annotator_OUTPUT/")
    ...
    cmd = f"    annotator {'run_cluster' if mode == 'cluster' else 'run_local'} --config {data_config_path}\n\n"
