...
def verif_species(species_id,path_augustus_config,rna_seq_directory) :
    '''
    ...
    if species_id not in liste_id_species:
        if rna_seq_directory != "":
            print(form("Warning, you don't provide a species id used by augusuts, only BRAKER will use ! If you want to launch Augustus too,\
             please add you training set in {path_augustus_config}species directory",'orange','bold'))
        else :
            raise ValueError(form(
                f"ERROR : You species id {species_id} isn't a id used by augustus and we haven't RNAseq for launch BRAKER only, please change your id or "
                f"add you training set in {path_augustus_config}species directory or add RNAseq data",
                'red', 'bold'))
